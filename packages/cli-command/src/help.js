...
function commandUsage(command) {
  let { name, definition } = command;
  let { usage, args, commands } = definition;

  if (!command.callback && commands?.length) {
    // if there are subcommands but no callback show subcommand usage
    usage ||= '<command>';
  } else {
    // show options followed by arguments
    args = args?.filter(a => !a.hidden && !a.deprecated)
      .map(a => a.required ? `<${a.name}>` : `[${a.name}]`);
    usage ||= `[options] ${args?.join(' ') || ''}`;
  }
...
async function visibleCommands(command, help) {
  ...
  for (let cmd of commands) {
    ...
    if (cmd.callback) {
      let { description } = cmd.definition;
...
function formatOptionHelp(option, usageWidth, maxWidth) {
  ...
  return option[0].padEnd(usageWidth) + wrapString(option[1], descWidth)
...
export async function formatHelp(command) {
  let { name, parent: top, definition } = command;
  let { description, examples } = definition;
  let output = [];
  ...
  let usageWidth = Math.max(
    args.reduce((max, [usage]) => Math.max(max, usage.length), 0),
    commands.reduce((max, [usage]) => Math.max(max, usage.length), 0),
    flags.reduce((max, [usage]) => Math.max(max, usage.length), 0)
  );
  ...
  let formatList = items => items.map(item => Array.isArray(item) ? (
    ...
    formatOptionHelp(item, usageWidth + 2, maxWidth - 2)
  ) : item).join('\n').replace(/^(?!$)/gm, ' '.repeat(2));
  ...
  examples = examples?.map(ex => ex.replace('$0', `$ ${name}`));
  if (examples?.length) output.push('Examples:', formatList(examples), '');
