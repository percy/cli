...
async function maybeParseCommand(input, parsed) {
  ...
  for (let name of names) {
    ...
    command = parent.definition.commands?.find(cmd => cmd.name === name);
    if (!command) break;
...
async function maybeParseFlag(input, parsed) {
  ...
  value = short && value?.startsWith('=') ? value.substr(1) : value;
...
async function maybeParseArg(input, parsed) {
  ...
  let index = parsed.__args_index__ ??= 0;

  // there are arguments left to parse
  if (index < parsed.command.definition.args?.length) {
...
async function maybeParseUnknown(input, parsed) {
  let { loose } = parsed.command.definition;
...
function findFlag(name, parsed, map) {
  ...
  let { flags } = parsed.command.definition;
...
function validateInclusiveFlag(flag, parsed) {
  ...
  let invalid = flag.inclusive
    .reduce((invalid, name) => findFlag(name, parsed, flag => (
...
function attributeName(option, parsed) {
  let { attribute, deprecated } = option;
  ...
  name ||= option.name ? camelcase(option.name) : option.short;

  // deprecated options can be mapped to other options
  if (deprecated) {
    let [until, recommend] = Array.isArray(deprecated) ? deprecated : (
      [typeof deprecated === 'string' ? deprecated : '']);

    // recommended replacement flag
    if (recommend?.startsWith('--')) {
    ...
    if (parsed.operators.has(option)) {
      ...
      parsed.log.deprecated(((
        `The '${usage}' ${flag ? 'option' : 'argument'} ` +
        `will be removed in ${until || 'a future release'}. `
      ) + (recommend || '')).trim());
    }
  }
