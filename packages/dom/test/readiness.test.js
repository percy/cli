...
describe('waitForReady', () => {
  ...
  it('skips the dom_stability check when dom_stability is false (kill-switch)', async () => {
    withExample('<p>kill-switch</p>', { withShadow: false });
    let result = await waitForReady({ stability_window_ms: 50, timeout_ms: 3000, image_ready: false, font_ready: false, network_idle_window_ms: 50, js_idle: false, dom_stability: false });
    expect(result.checks.dom_stability).toBeUndefined();
    expect(result.passed).toBe(true);
  });

  it('accepts the camelCase domStability flag', async () => {
    withExample('<p>kill-switch-camel</p>', { withShadow: false });
    let result = await waitForReady({ stabilityWindowMs: 50, timeoutMs: 3000, imageReady: false, fontReady: false, networkIdleWindowMs: 50, jsIdle: false, domStability: false });
    expect(result.checks.dom_stability).toBeUndefined();
    expect(result.passed).toBe(true);
  });

  it('still runs dom_stability when dom_stability is true (default behavior)', async () => {
    withExample('<p>default</p>', { withShadow: false });
    let result = await waitForReady({ stability_window_ms: 50, timeout_ms: 3000, image_ready: false, font_ready: false, network_idle_window_ms: 50, js_idle: false, dom_stability: true });
    expect(result.checks.dom_stability).toBeDefined();
    expect(result.checks.dom_stability.passed).toBe(true);
  });
