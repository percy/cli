...
export const wait = command('wait', {
  ...
  examples: [
    '$0 --build 2222222',
    '$0 --project org/project --commit HEAD'
  ],
...
function failureMessage(type, {
  ...
  switch (type) {
    case 'render_timeout':
      return 'Some snapshots in this build took too long ' +
        'to render even after multiple retries.';
    case 'no_snapshots':
      return 'No snapshots were uploaded to this build.';
    case 'missing_finalize':
      return 'Failed to correctly finalize.';
    case 'missing_resources':
      return missingParallel
        ? `Only ${parallelCount} of ${parallelTotal} parallel builds were received.`
