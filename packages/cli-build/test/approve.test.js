...
describe('percy build:approve', () => {
  ...
  it('successfully approves a build with valid credentials', async () => {
    process.env.PERCY_TOKEN = '<<PERCY_TOKEN>>';
    process.env.PERCY_FORCE_PKG_VALUE = JSON.stringify({ name: '@percy/client', version: '1.0.0' });
    process.env.PERCY_USERNAME = 'test-username';
    process.env.PERCY_ACCESS_KEY = 'test-access-key';
    ...
    api.reply('/reviews', (req) => {
      ...
      expect(req.headers['bstack-username']).toEqual('test-username');
      expect(req.headers['bstack-access-key']).toEqual('test-access-key');
      return [200, { success: true }];
    });
  ...
  it('logs an error when build approval fails with 500 Internal Server Error', async () => {
    process.env.PERCY_TOKEN = '<<PERCY_TOKEN>>';
    process.env.PERCY_FORCE_PKG_VALUE = JSON.stringify({ name: '@percy/client', version: '1.0.0' });
    process.env.PERCY_USERNAME = 'test-username';
    process.env.PERCY_ACCESS_KEY = 'test-access-key';
    ...
    api.reply('/reviews', (req) => {
      ...
      return [500, { errors: [{ detail: 'Internal Server Error' }] }];
    });

    await expectAsync(approve(['123'])).toBeRejected();

    expect(logger.stderr).toEqual([
      '[percy] Error: Internal Server Error',
      '[percy] Error: Failed to approve the build'
    ]);
  });
});
