...
export class ProxyDetector {
  ...
  async detectProxy(options = {}) {
    ...
    if (platform === 'darwin') {
      const sys = this.#detectMacOSProxy();
      if (sys) discovered.set(sys.url, { source: sys.source });
    } else if (platform === 'linux') {
      const sys = this.#detectLinuxProxy();
      if (sys) discovered.set(sys.url, { source: sys.source });
    } else if (platform === 'win32') {
      const sys = this.#detectWindowsProxy();
      if (sys) discovered.set(sys.url, { source: sys.source });
    }

    // ── Layer 3: Response-header fingerprinting ──────────────────────────────
    const headerFindings = checkHeaders ? await this.#detectViaHeaders(timeout) : [];
    ...
    const procFindings = scanProcesses ? await this.#detectProxyProcesses() : [];
    for (const pf of procFindings) findings.push(pf);

    // ── Layer 5: WPAD / DNS auto-discovery ───────────────────────────────────
    const wpadFindings = checkWpad ? await this.#detectWpad() : [];
  ...
  #detectMacOSProxy() {
    try {
      const out = cp.execSync('scutil --proxy', { timeout: 5000, encoding: 'utf8' });
  ...
  async #detectViaHeaders(timeout) {
    ...
    for (const target of PROBE_TARGETS) {
      try {
        const result = await httpProber.probeUrl(target, { timeout });
        /* istanbul ignore next */
  ...
  async validateProxy(proxyUrl, timeout) {
    ...
    const results = await Promise.all(
      TEST_URLS.map(u => httpProber.probeUrl(u, { proxyUrl, timeout }))
    );
