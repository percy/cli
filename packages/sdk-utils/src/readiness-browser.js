...
async function runAllChecks(config, result, aborted) {
  ...
  if (config.dom_stability !== false && config.stability_window_ms > 0) { expected.push('dom_stability'); checks.push(checkDOMStability(config.stability_window_ms, aborted).then(r => { result.checks.dom_stability = r; })); }
